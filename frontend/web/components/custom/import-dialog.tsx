...
import type { ImportUsersResult } from "@/app/lib/api";
...
export interface ImportDialogProps {
	...
	description: string;
	fileInputId: string;
	onImport: (file: File, dryRun: boolean) => Promise<ImportUsersResult>;
...
export function ImportDialog({
	...
	async function handlePreview() {
		...
		setError(null);
		try {
			const r = await onImport(file, true);
			setResult(r);
		} catch (e) {
	...
	async function handleImport() {
		...
		setError(null);
		try {
			const r = await onImport(file, false);
			setResult(r);
			if (r.created > 0) {
				router.refresh();
			}
		...
		>
			...
			<DialogContent className="max-w-lg">
				...
				<FieldGroup>
					{extraFields}
					<Field>
						<FieldLabel htmlFor={fileInputId}>CSV file</FieldLabel>
						<input
							ref={fileRef}
							id={fileInputId}
							type="file"
