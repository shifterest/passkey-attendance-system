...
import { type ImportUsersResult, importOrgs } from "@/app/lib/api";
...
export function ImportOrgsDialog({ trigger }: ImportOrgsDialogProps) {
	...
	async function handlePreview() {
		...
		setError(null);
		try {
			const r = await importOrgs(file, true);
			setResult(r);
		} catch (e) {
	...
	async function handleImport() {
		...
		setError(null);
		try {
			const r = await importOrgs(file, false);
			setResult(r);
			if (r.created > 0) {
				router.refresh();
			}
		...
		>
			...
			<DialogContent className="max-w-lg">
				<DialogHeader>
					...
					<DialogDescription>
						Upload a CSV with columns: name, description (optional).
					</DialogDescription>
				</DialogHeader>
				<FieldGroup>
					<Field>
						<FieldLabel htmlFor="import-orgs-file">CSV file</FieldLabel>
						<input
							ref={fileRef}
							id="import-orgs-file"
							type="file"
