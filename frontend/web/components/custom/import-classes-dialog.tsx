...
import { type ImportUsersResult, importClasses } from "@/app/lib/api";
...
export function ImportClassesDialog({ trigger }: ImportClassesDialogProps) {
	...
	async function handlePreview() {
		...
		setError(null);
		try {
			const r = await importClasses(file, true);
			setResult(r);
		} catch (e) {
	...
	async function handleImport() {
		...
		setError(null);
		try {
			const r = await importClasses(file, false);
			setResult(r);
			if (r.created > 0) {
				router.refresh();
			}
		...
		>
			...
			<DialogContent className="max-w-lg">
				<DialogHeader>
					...
					<DialogDescription>
						Upload a CSV with columns: course_code, course_name, teacher_email.
						Schedule can be added after import.
					</DialogDescription>
				</DialogHeader>
				<FieldGroup>
					<Field>
						<FieldLabel htmlFor="import-classes-file">CSV file</FieldLabel>
						<input
							ref={fileRef}
