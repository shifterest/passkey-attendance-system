...
- Propose one next task at a time, wait for approval, implement fully, then stop.
- After implementation, provide a short change delta by default; provide detailed explanation only when explicitly requested.
- Before starting the next task, require user confirmation of behavioral or architectural understanding.
...
3. Play Integrity is required for check-in and login in internet-connected deployments.
4. BLE proximity uses bucketed RSSI scoring (not linear):
   - `> -65` => +7
   - `-65 to -80` => +4
   - `-80 to -90` => +2
   - `< -90` => 0
   - Teacher kiosk is the beacon; student app scans and reports RSSI.
...
- Assurance bands:
  - **≥20 (High):** auto-confirmed, green indicator, no teacher action needed
  - **10–19 (Standard):** auto-accepted, neutral indicator
  - **<10 (Flagged):** held; teacher must explicitly confirm or reject
...
- Teachers may open a `presence_check` window (via `window_type` on `CheckInSession`) at any time after the attendance window closes; results are supplemental and do not modify original attendance status.
- Only one `attendance`-type window may be open per class session at a time; `presence_check` windows have no such constraint.
- `CheckInSession` records are auto-created from `Class.schedule` data at the scheduled class time; teacher-initiated sessions exist as an override for ad-hoc cases.
...
- `Class.schedule` is JSON list data, not a separate schedule table.
